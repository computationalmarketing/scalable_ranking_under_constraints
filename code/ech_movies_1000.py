...
# Code running ranking of 1000 movies
...
PATH_RESULTS = '../results/ml-25m-1000'
...
def extract_data(user, top_k=1000, sample_size=1000):
    ...
    # b_k list
    b = np.sum(exposure)*np.array([0.015, 0.015, 0.015, 0.015, 0.0])
...
for u in tqdm(users_train[:10]):
    U, A_list, b, X = extract_data(u)
    lmbd = user_dual(U, A_list, b)
    for e in eps:
        P, S = optimal_ranking(U, A_list, lmbd, method='rearrangement',EPSILON=e)
...
for u in tqdm(range(N_USERS)):
    ...
    start = time.time()
    lmbd = user_dual(U, A_list, b)
    P, S = optimal_ranking(U, A_list, lmbd, method='rearrangement',EPSILON=EPSILON)
    end = time.time()
...
for u in tqdm(users_test):
    ...
    start = time.time()
    P, S = optimal_ranking(U, A_list, lmbd_avg, method='rearrangement',EPSILON=EPSILON)
    end = time.time()
